...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | ReadNum nm -> 
     (* exec (Assign (nm, Val (Int (read_number ())))) stt 
        or we can write the following: *)
     (nm, (Int (read_number ()))) :: stt
  | WriteNum e -> 
     (match eval e stt with
      | Bool _ -> raise (Failure "Type error on Write")
      | Int i -> write_number i; stt
     )
  | IfThen (cond, body) ->
     (match eval cond stt with
      | Int _ -> raise (Failure "Type error on IfThen")
      | Bool false -> stt
