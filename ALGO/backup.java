...
public class DelivC {
	...
	Node left;
	Node right;
	int shortestTour = 999999;
	int randomInterval = 20;
	int restartInterval = 500000;
...
( 0, inputFileName.length()-4 ); // Strip off ".txt"
	...
	private Tour makeTour(Graph gp, Node Nde){
		...
		Tour tp = new Tour(shortestTour, randomInterval, restartInterval);
		
		for(int i = 0; i <= nde.getOutgoingEdges().size() - 1; i++){ 
			tp.addNodeTourNodes(nde);
			makeTour(gp, nde.getOutgoingEdges.().get(i));
					
		}
		tp.addNodeTourNodes(getTourList(0));
		return tp;	
	}

	private void localSrchBig(Graph gp){
		//find a random int to swap with current
		int randomStart = (int)(Math.random()*(gp.getNodeList().size - 1);
		t = makeTour(gp.getOutGoingNodes(random);
		t.setNextRandomRestart(restartInterval);
		greedyDescent(gp, t);
	}
	
	private int greedyDescent(Graph gp, Tour t){	
		int tourLength1 = walk(gp, t);
		int tourLength2 = walk(gp, t);
		if(tourLength1 < tourLength2){
			if(tourLength1 < shortestTour){
				shortestTour = tourLength1;
				t.setNextRandomRestart(restartInterval);
			}
			return tourLength1;
		}else{
			if(tourLength2 < shortestTour){
				shortestTour = tourLength2;
				t.setNextRandomRestart(0);
			}
			return tourLength2;
		}			
	}	
	private Tour walk(Tour t){
		...
		t.setNextRandomRestart(t.getNextRandomRestart - 1);

		randomLeft = (int)(Math.random()*(size-3) + 1);
		randomRight = (int)(Math.random()*(size -3) + 1);
		if(t.getNextRandomWalk(t.getNextRandWalk == 0){
			lef	
		}				
		return t;
	}
